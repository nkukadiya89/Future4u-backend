...
class Command(BaseCommand):
    help = "Write sample domain CSV and/or load domains from CSV (idempotent skips invalid rows via bulk import)."

    def add_arguments(self, parser):
        parser.add_argument(
            "--sample-path",
            default="domain_master_sample.csv",
            help="Path to write sample CSV (default: domain_master_sample.csv).",
        )
        parser.add_argument(
            "--no-sample",
            action="store_true",
    ...
    def handle(self, *args, **options):
        if not options.get("no_sample"):
            ...
            sample_path.write_bytes(domain_service.sample_csv_bytes())
        ...
        logger.info("init_domain_master loading %s rows from %s", len(rows), path)
