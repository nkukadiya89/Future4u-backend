...
from recommendation.profiles.parent.chat_service import ParentAIChatService
from recommendation.profiles.parent.service import ParentAIRecommendationService
from utils.throttles import RecommendationRateThrottle, AIChatRateThrottle

logger = logging.getLogger(__name__)


class ParentAIRecommendationAPIView(APIView):
    """
    GET /api/parent/ai-recommendations/{assessment_id}/
    Generates AI career recommendations from a completed parent assessment.
    """
    ...
    def get(self, request, assessment_id, *args, **kwargs):
        try:
            data = ParentAIRecommendationService().generate(
...
class ParentAIRecommendationChatAPIView(APIView):
    """
    GET /api/parent/ai-recommendations/{assessment_id}/chat/?suggestion_id=1
    Returns selected career context and suggested chips without calling AI.

    POST /api/parent/ai-recommendations/{assessment_id}/chat/
    Career-specific assistant for a saved parent recommendation suggestion.
    """
    ...
    def get(self, request, assessment_id, *args, **kwargs):
        try:
            data = ParentAIChatService().context(
                user=request.user,
                assessment_id=assessment_id,
