...
class CompanyViewSet(SearchOrderingFilter, ModelViewSet):
    ...
    def send_email(self, user, context):
        if user:
            send_mail(
                "PROCEM Security Alert For Create New Password",
                "reset-pass.html",
                context,
            )
        ...
        if serializer.is_valid():
            with transaction.atomic():
                ...
                end_date = start_date + timedelta(
                    days=int(welcome_package.duration)  # type: ignore
                )
                ...
                email_thread = threading.Thread(
                    target=self.send_email, args=(instance, context)
                )
                email_thread.start()
                return Response(
                    {
    ...
    def update(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            serializer = CompanyInfoSerializer(comapny)
            return Response(
                {"success": True, "data": serializer.data},
