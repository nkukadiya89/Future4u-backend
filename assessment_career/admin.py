...
# ── Student Career Recommendation (existing) ─────────────────────────────────
...
# ── Parent Career Recommendation ────────────────────────────────────────────

class ParentCareerRecommendationSuggestionInline(admin.TabularInline):
...
class ParentCareerRecommendationChatMessageInline(ReadOnlyAdminMixin, admin.TabularInline):
    model = ParentCareerRecommendationChatMessage
    extra = 0
    fields = ("id", "role", "content", "created_at")
    readonly_fields = ("id", "role", "content", "created_at")
    can_delete = False
...
@admin.register(ParentCareerRecommendation)
class ParentCareerRecommendationAdmin(admin.ModelAdmin):
    list_display = (
        "id",
        "user",
        "assessment",
    ...
    inlines = [ParentCareerRecommendationSuggestionInline]
...
@admin.register(ParentCareerRecommendationSuggestion)
class ParentCareerRecommendationSuggestionAdmin(admin.ModelAdmin):
    list_display = (
        "id",
        "recommendation",
        "career_name",
...
@admin.register(ParentCareerRecommendationChatSession)
class ParentCareerRecommendationChatSessionAdmin(admin.ModelAdmin):
    list_display = ("id", "suggestion", "message_count", "updated_at", "created_at")
    search_fields = (
        "suggestion__career_name",
        "suggestion__recommendation__user__email",
    ...
    raw_id_fields = ("suggestion",)
    readonly_fields = ("summary", "created_at", "updated_at")
    inlines = [ParentCareerRecommendationChatMessageInline]
