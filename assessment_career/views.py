...
from .serializers import (
    ...
    ParentCareerRecommendationSerializer,
    ParentCareerRecommendationSuggestionSerializer,
)
...
class ParentCareerSuggestionViewSet(ModelViewSet):
    serializer_class = ParentCareerRecommendationSerializer
    filter_backends = [SearchFilter, OrderingFilter]
    pagination_class = Pagination
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTAuthentication]
    ...
    def get_queryset(self):
        queryset = ParentCareerRecommendation.objects.filter(
            deleted=False, user=self.request.user
        ).select_related("assessment", "user").prefetch_related("suggestions").order_by("-id")
        assessment_id = self.request.query_params.get("assessment_id")
        if assessment_id:
            queryset = queryset.filter(assessment_id=assessment_id)
        return queryset
    ...
    def list(self, request, *args, **kwargs):
        ...
        serializer = ParentCareerRecommendationSerializer(queryset, many=True)
        return Response({"success": True, "data": serializer.data})
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if not instance:
            return Response(
                {"success": False, "message": "No ParentCareerRecommendation matches the given query"},
    ...
    def compare(self, request):
        ...
        suggestions = ParentCareerRecommendationSuggestion.objects.filter(
            id__in=suggestion_ids, recommendation__user=request.user, deleted=False
        ).order_by("display_order")
        ...
        serializer = ParentCareerRecommendationSuggestionSerializer(suggestions, many=True)
        return Response(
            {"success": True, "data": serializer.data},
            status=status.HTTP_200_OK,
        )
...
class ParentCareerSuggestionDetailViewSet(BaseModelViewSet):
    serializer_class = ParentCareerRecommendationSuggestionSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        return ParentCareerRecommendationSuggestion.objects.filter(
            deleted=False,
            recommendation__user=self.request.user,
        )
