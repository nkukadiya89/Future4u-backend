...
class LanguageViewSet(ModelViewSet):
    ...
    def get_object(self):
        ...
        try:
            ...
            raise NotFound()
        try:
            return language_service.language_base_queryset().filter(deleted=False).get(pk=pk)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"success": True, "message": "Language created", "data": serializer.data},
    ...
    def dropdown(self, request, *args, **kwargs):
        ...
        return Response({"success": True, "data": LanguageDropdownSerializer(qs, many=True).data})
    ...
    def change_status(self, request, pk=None, *args, **kwargs):
        ...
        language_service.set_active_status(language=instance, user=request.user, is_active=ser.validated_data["is_active"])
    ...
    def bulk_archive(self, request, *args, **kwargs):
        ...
        if not ser.is_valid():
            return Response({"success": False, "message": ser.errors}, status=status.HTTP_400_BAD_REQUEST)
        n = language_service.bulk_archive(ids=list(ser.validated_data["ids"]), user=request.user)
    ...
    def bulk_restore(self, request, *args, **kwargs):
        ...
        if not ser.is_valid():
            return Response({"success": False, "message": ser.errors}, status=status.HTTP_400_BAD_REQUEST)
        n = language_service.bulk_restore(ids=list(ser.validated_data["ids"]), user=request.user)
    ...
    def bulk_upload(self, request, *args, **kwargs):
        ...
        if not rows:
            return Response({"success": False, "message": parse_errors or ["No data rows."]}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def import_logs(self, request, *args, **kwargs):
        ...
        if page is not None:
            return self.get_paginated_response({"success": True, "data": LanguageImportBatchSerializer(page, many=True).data})
        return Response({"success": True, "data": LanguageImportBatchSerializer(qs, many=True).data})
