...
class Command(BaseCommand):
    help = "Write sample skill CSV and/or load skills from CSV."

    def add_arguments(self, parser):
        parser.add_argument(
            "--sample-path",
            default=str(Path(settings.BASE_DIR) / "core" / "management" / "source" / "skill_master_sample.csv"),
        ...
        parser.add_argument(
            ...
            dest="load_path",
            default=None,
            help="Load skills from CSV at this path.",
        )
        parser.add_argument(
            "--username",
            default=None,
    ...
    def handle(self, *args, **options):
        if not options.get("no_sample"):
            ...
            sample_path.write_bytes(skill_service.sample_csv_bytes())
        ...
        logger.info("init_skills loading %s rows from %s", len(rows), load_path)
