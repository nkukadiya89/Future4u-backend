...
class Command(BaseCommand):
    ...
    def _parse_row(self, r: dict, idx: int, col_flags: dict) -> dict | None:
        ...
        try:
            signal_strength = (
                ...
                if col_flags["has_signal_strength_column"]
                else 1
            )
        ...
        domain_codes = [
            code.strip() for code in mapped_domains_raw.split(sep) if code.strip()
        ]

        question_type = (
            (r.get("question_type") or "scale").strip().lower()
            if col_flags["has_question_type_column"]
            else "scale"
        )
        ...
        education_level_code = (
            ...
            if col_flags["has_education_level_column"]
            else ""
        )
        target_stream_code = (
            (r.get("target_stream") or "").strip()
            if col_flags["has_target_stream_column"]
            else ""
        )
        sequence_order_raw = (
            (r.get("sequence_order") or "0").strip()
            if col_flags["has_sequence_order_column"]
            else "0"
        )
        try:
        ...
        return {
            ...
            "option_texts": [
                (r.get(f"option_{i}_text") or "").strip() for i in range(1, 5)
            ],
        }
    ...
    def _resolve_relations(self, parsed: dict, idx: int) -> dict | None:
        ...
        if parsed["domain_codes"]:
            ...
            missing_codes = [
                c for c in parsed["domain_codes"] if c.lower() not in found_codes
            ]
            if missing_codes:
                self.stdout.write(
                    self.style.WARNING(
        ...
        if parsed["mapped_streams_raw"]:
            ...
            for sc in [
                s.strip() for s in parsed["mapped_streams_raw"].split("|") if s.strip()
            ]:
    ...
    def _create_or_update_question(
        self, parsed: dict, relations: dict, col_flags: dict
    ) -> tuple:
        defaults = {
        ...
        if not q and not (
            col_flags["has_sequence_order_column"] and parsed["sequence_order"]
        ):
            q = Question.objects.filter(
                dimension=parsed["dim"], question_text=parsed["qt"]
            ).first()
        ...
        if q:
            ...
            if (
                col_flags["has_signal_strength_column"]
                and q.signal_strength != parsed["signal_strength"]
            ):
            ...
            if q.education_level_id != (
                relations["education_level_obj"].id
                if relations["education_level_obj"]
                else None
            ):
                q.education_level = relations["education_level_obj"]
                changed_fields.append("education_level")
            if q.target_stream_id != (
                relations["target_stream_obj"].id
                if relations["target_stream_obj"]
                else None
            ):
        ...
        q = Question.objects.create(
            dimension=parsed["dim"], question_text=parsed["qt"], **defaults
        )
        return q, True, True
    ...
    def _load_from_csv(self, *, load_path: Path, dry_run: bool):
        ...
        with load_path.open("r", newline="", encoding="utf-8-sig") as f:
            ...
            if not all(
                ...
                raise ValueError(
                    "Missing option headers. Expected option_1_text .. option_4_text columns."
                )

            missing = [
                h
                for h in ("dimension", "question_text", "is_active")
            ...
            for idx, r in enumerate(reader, start=2):
                ...
                if dry_run:
                    dry_run_count += 1
                    dry_run_dimensions[parsed["dim"]] = (
                        dry_run_dimensions.get(parsed["dim"], 0) + 1
                    )
                    continue

                q, was_created, did_update = self._create_or_update_question(
                    parsed, relations, col_flags
                )
                if was_created:
                    created_q += 1
                elif did_update:
    ...
    def handle(self, *args, **options):
        ...
        load_path = (
            Path(options["load_path"]) if options.get("load_path") else sample_path
        )
