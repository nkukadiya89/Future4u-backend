...
class Command(BaseCommand):
    help = "Write sample domain-career-mapping CSV and/or load mappings from CSV."

    def add_arguments(self, parser):
        parser.add_argument(
            "--sample-path",
            default=str(
                Path(settings.BASE_DIR) / "core" / "management" / "source" / "domain_career_mapping_sample.csv"
            ),
        ...
        parser.add_argument(
            "--load",
            dest="load_path",
            default=str(Path(settings.BASE_DIR) / "core" / "management" / "source" / "domain_career_mapping.csv"),
            help="Load domain-career mappings from CSV at this path.",
        )
        parser.add_argument(
            "--username",
            default=None,
    ...
    def handle(self, *args, **options):
        if not options.get("no_sample"):
            ...
            sample_path.write_bytes(domain_career_mapping_service.sample_csv_bytes())
        ...
        logger.info("init_domain_career_mappings loading %s rows from %s", len(rows), load_path)
