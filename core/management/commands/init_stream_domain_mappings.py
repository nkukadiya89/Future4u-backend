...
class Command(BaseCommand):
    help = "Write sample stream-domain-mapping CSV and/or load mappings from CSV."

    def add_arguments(self, parser):
        parser.add_argument(
            "--sample-path",
            default=str(
                Path(settings.BASE_DIR) / "core" / "management" / "source" / "stream_domain_mapping_sample.csv"
            ),
    ...
    def handle(self, *args, **options):
        if not options.get("no_sample"):
            ...
            sample_path.write_bytes(stream_domain_mapping_service.sample_csv_bytes())
        ...
        logger.info("init_stream_domain_mappings loading %s rows from %s", len(rows), load_path)
