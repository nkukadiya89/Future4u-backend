...
class ParentAssessment(BaseModule):
    ...
    concerns = models.ManyToManyField(Concern, blank=True)
    parent_career_expectations = models.ManyToManyField(
        ParentCareerExpectation, blank=True
    )
...
class ProfessionalAssessment(BaseModule):
    ...
    salary_expectation = models.CharField(
        max_length=50,
        choices=SalaryExpectation.choices,
        null=True,
        blank=True,
    )
    timeline = models.CharField(
        max_length=50, choices=Timeline.choices, null=True, blank=True
    )
