...
DOMAIN_KNOWLEDGE = {
    ...
    "ecommerce": (
        "E-commerce is one of the fastest-growing sectors globally — and it needs people who understand both technology and consumer behaviour.",
        "Margins are thin and competition is brutal. Most e-commerce businesses fail within the first two years.",
        "Pick one product category, build a small Shopify or WooCommerce store, and run it for 30 days — the operational learning is worth more than any course.",
        "The tension: working at a large marketplace (stable, slower, process-heavy) vs. building your own D2C brand (riskier, more creative, higher upside).",
    ),
    "entrepreneurship": (
...
def _dk(code: str) -> tuple:
    ...
    try:
        ...
        pass
    return DOMAIN_KNOWLEDGE.get(clean, _DEFAULT_KNOWLEDGE)
    ...
    *,
    ...
    if recommendation_type == "stream":
        ...
        if not code:
            return _no_data_message(level_code)
        k = _sk(code)
        stream_label = _d(code, STREAM_DISPLAY)
    ...
    if recommendation_type == "college_domain":
        ...
        if not code:
            return _no_data_message(level_code)
        k = _dk(code)
        domain_label = _d(code, DOMAIN_DISPLAY)
    ...
    if recommendation_type == "career" and top_domain:
        k = _dk(top_domain)
        domain_label = _d(top_domain, DOMAIN_DISPLAY)
        career_label = _d(top_career, CAREER_DISPLAY) if top_career else None
        second = domain_ranking[1] if len(domain_ranking) > 1 else None
        second_label = second.get("domain_name") if second else None
