...
def _empty(message: str) -> dict[str, Any]:
    return {"message": message, "tier": "unknown",
            "recommended_streams": [], "top_domains": [],
            "top_careers": [], "skill_gaps": [], "next_step": None}
...
def _estimate_proficiency(*, skill_name: str, dim_scores: dict[str, float]) -> int:
    ...
    domain_kw = ("marketing","finance","design","health","medical","communication","writing","business","management")
    if any(k in name for k in technical):
        basis = aptitude
    elif any(k in name for k in domain_kw):
        basis = interest
    else:
        basis = aptitude * 0.45 + interest * 0.45 + dim_scores.get("personality", 0.0) * 0.05 + dim_scores.get("work_style", 0.0) * 0.05
...
def _recommend_streams(*, dim_scores: dict[str, float]) -> list[dict[str, Any]]:
    ...
    all_streams = {
        s.stream_code: s
        for s in Stream.objects.filter(is_active=True, deleted=False,
    ...
    for code, score in sorted(stream_scores.items(), key=lambda x: -x[1]):
        ...
        results.append({
            ...
            "traditional_equivalent": getattr(s, "traditional_equivalent", "") or "",
            "reason": f"Suits your {top_dim} strength",
        })
    return results[:6]


def _score_domains_from_stream_codes(*, stream_codes: list[str], dim_scores: dict[str, float]) -> tuple[list[dict], dict, set]:
    rows = list(
    ...
    for did, stream_w in best_stream_weight_by_domain.items():
        ...
        final = _domain_fit_score(domain_code=domain_code, dim_scores=dim_scores, stream_weight=stream_w)
        score_by_id[did] = float(final)

        affinity = _DOMAIN_AFFINITY.get(domain_code, {dim: 0.25 for dim in DIMENSIONS})
        top_dim = max(affinity, key=lambda k: dim_scores.get(k, 0.0) * affinity.get(k, 0.0))
...
def _domain_fit_score(*, domain_code: str, dim_scores: dict[str, float], stream_weight: int) -> float:
    """
...
def _score_domains(*, ctx: _UserContext, dim_scores: dict[str, float]) -> tuple[list[dict], dict, set]:
    rows = list(
        StreamDomainMapping.objects.filter(
    ...
    for m in rows:
        ...
        rel_note = (" · high future relevance" if rel and rel >= 80
                    else " · niche future relevance" if rel and rel <= 40 else "")
        reason = f"Strong {dim_label} match{rel_note}".capitalize()

        items.append({"id": str(d.pk), "name": getattr(d, "domain_name", "") or "",
                      "score": round(float(final), 2), "reason": reason})
...
def _score_careers(*, top_domain_pk_set: set, domain_score_by_id: dict,
                   top_dim: str, edu_seq: int, filter_by_edu: bool) -> list[dict]:
    dim_label = {"interest": "interest", "aptitude": "aptitude",
                 "personality": "personality", "work_style": "work style"}.get(top_dim, top_dim)

    career_filter: dict[str, Any] = dict(
        domain_id__in=top_domain_pk_set, deleted=False, is_active=True,
        domain__deleted=False, domain__is_active=True,
...
def _score_skill_gaps(*, top_domain_pk_set: set, user_id: int, dim_scores: dict[str, float]) -> list[dict]:
    skill_rows = list(
        DomainSkillMapping.objects.filter(
            domain_id__in=top_domain_pk_set, deleted=False, is_active=True,
            domain__deleted=False, domain__is_active=True,
            skill__deleted=False, skill__is_active=True,
        ).select_related("skill").only("skill_id", "weight_score", "skill__skill_name")
    )
    req: dict[Any, int] = {}
    names: dict[Any, str] = {}
    for m in skill_rows:
    ...
    user_skills = {
        ...
        for r in UserSkill.objects.filter(
            user_id=user_id, deleted=False, is_active=True,
            skill__deleted=False, skill__is_active=True,
    ...
    for sid, rw in req.items():
        prof = user_skills.get(sid) or _estimate_proficiency(skill_name=names.get(sid, ""), dim_scores=dim_scores)
...
def generate_recommendation(user_id: int) -> dict[str, Any]:
    ...
    if seq <= TIER_10TH:
        ...
        strong_top_careers = [c for c in top_careers if float(c.get("score", 0.0)) >= 40.0]
        medium_high_skill_gaps = [g for g in skill_gaps if g.get("gap_level") in {"HIGH", "MEDIUM"}]
    ...
    # ── TIER 3: ITI / Diploma ─────────────────────────────────────────────────
    if seq <= TIER_DIPLOMA:
        top_domains, domain_score_by_id, top_domain_pk_set = _score_domains(ctx=ctx, dim_scores=dim_scores)
        if not top_domains:
            return _empty("No domain mappings found for your stream.")
        top_careers = _score_careers(
            top_domain_pk_set=top_domain_pk_set, domain_score_by_id=domain_score_by_id,
            top_dim=top_dim, edu_seq=seq, filter_by_edu=True,
        )
        return {
            "tier": "diploma",
    ...
    top_careers = _score_careers(
        top_domain_pk_set=top_domain_pk_set, domain_score_by_id=domain_score_by_id,
        top_dim=top_dim, edu_seq=seq, filter_by_edu=False,
    )
    skill_gaps = _score_skill_gaps(
    ...
    message = "ok" if has_assessment else "Complete the assessment for more accurate recommendations."
