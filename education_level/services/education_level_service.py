...
def bulk_import_rows(
    *, user, rows: list[dict], serializer_class, context: dict
    ...
    codes = [
        (r.get("level_code") or "").strip().lower() for r in rows if r.get("level_code")
    ]
    if codes:
        existing = {
            obj.level_code: obj.sequence_order
            for obj in EducationLevel.objects.filter(level_code__in=codes).only(
                "level_code", "sequence_order"
            )
        }
