...
class GroupViewSet(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        user = request.user
        no_pagination = self.request.query_params.get("no_pagination")  # type: ignore
        company_id = self.request.query_params.get("company_id")  # type: ignore
        vendor_id = self.request.query_params.get("vendor_id")  # type: ignore
        if company_id:
            queryset = self.filter_queryset(
                CustomGroup.objects.filter(company=company_id, deleted=0).order_by(
                    "-id"
                )
            )
            ...
            queryset = self.filter_queryset(
                CustomGroup.objects.filter(vendor=vendor_id, deleted=0).order_by("-id")
            )
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.validated_data["created_by"] = request.user  # type: ignore
            serializer.save()
...
class PermissionViewSet(ModelViewSet):
    ...
    def get_purticlare_permission(self, request, *args, **kwargs):
        content_types = self.request.query_params.get("content_types")  # type: ignore
        model_names = self.request.query_params.get("model_names")  # type: ignore
    ...
    def group_wise_permission(self, request, *args, **kwargs):  # type: ignore
        ...
        if group_ids:
            ...
            try:
                if role_name:
                    ...
                    user_assigned_groups = Group.objects.get(name="Vendor Admin")
    ...
    def list(self, request, *args, **kwargs):
        group_name = self.request.query_params.get("group_name")  # type: ignore
...
class CreateGroupWithPermissionsViewSet(ModelViewSet):
    ...
    def get_group_permission_by_user(self, request, *args, **kwargs):
        ...
        if login_user.company:  # type: ignore
            user_exclude_groups = ["Company Admin", "Company Admin PR Rights"]
        elif login_user.vendor:
            user_exclude_groups = ["Vendor Admin"]
    ...
    def create(self, request, *args, **kwargs):
        ...
        if company_id:
            ...
            custom_group.save()
            user = User.objects.get(id=request.user.id).id  # type: ignore
            custom_group.user_set.add(user)
        ...
        return Response(
            {
                ...
                "id": custom_group.id,  # type: ignore
                "group_name": group_name,
