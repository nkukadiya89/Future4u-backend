...
class VerifiedOTPViewSet(ModelViewSet):
    queryset = EmailPhoneVerify.objects.filter(deleted=False).order_by("-id")
    ...
    def phone_number_verify(self, request, *args, **kwargs):
        ...
        if phone_number:
            ...
            message = (
                f"dev.eswitch.ai {phone_number_otp} is the verification "
    ...
    def resend_otp(self, request, *args, **kwargs):
        ...
        if email:
            ...
            try:
                ...
                sms_contacts = int(str(contacts)[2:])
                message = (
                    f"dev.eswitch.ai {phone_number_otp} is the verification code "
