...
from course_generation.prompts.course_generation_prompt import (
...
class CourseGenerator:
    ...
    def generate(
        ...
        raise last_error or CourseGenerationValidationError(
            "Course generation failed",
            error="Validation failed",
            details="Course generation failed after retries",
        )
        ...
        *,
        ...
        try:
            ...
            if not raw_text or not raw_text.strip():
                raise CourseGenerationValidationError(
            ...
            logger.warning("Course generation JSON parse failed: %s", exc)
            raise CourseGenerationValidationError(
                "Unable to generate course details. Please try again.",
                error="Generation failed",
                details="AI response could not be parsed as valid JSON",
            ) from exc
        except (ValidationError, ValueError) as exc:
            logger.warning("Course generation validation failed: %s", exc)
            raise CourseGenerationValidationError(
                str(exc),
                error="Validation failed",
                details=str(exc),
            ) from exc
        except CourseGenerationValidationError:
            raise
...
def _format_llm_error(exc: Exception) -> str:
    ...
    if "insufficient_quota" in lowered or "quota" in lowered or "429" in lowered:
        return "Course generation is busy right now. Please try again shortly."
    return "Unable to generate course details right now. Please try again."


def _is_retryable_generation_error(exc: CourseGenerationValidationError) -> bool:
