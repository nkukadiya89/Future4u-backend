    ...
    try:
        ...
        for device in devices:
            try:
                ...
                total_usage_30_days = float(latest_energy.value) - float(
                    first_energy.value
                )
                ...
                if recent_usage < expected_70_percent_usage:
                    print(
                        f"📊 Device {device.device_code}: Recent usage {recent_usage:.2f} < 70% of daily average {daily_average:.2f}"
                    )
                    ...
                    alert_sent = send_meter_usage_alert_email(
                        ...
                        usage_percentage=(
                            (recent_usage / daily_average) * 100
                            if daily_average > 0
                            else 0
                        ),
                    )
                    ...
                    if alert_sent:
                        alerts_sent += 1
                        print(
                            f"📧 Meter usage alert sent for device: {device.device_code}"
                        )
                    else:
                        print(
                            f"❌ Failed to send meter usage alert for device: {device.device_code}"
                        )

            except Exception as e:
                print(f"❌ Error checking device {device.device_code}: {str(e)}")
                logger.error(
                    f"Error checking meter usage for device {device.device_code}: {str(e)}"
                )
                continue
...
def check_device_meter_usage(device_id):
    ...
    try:
        ...
        # Get company name from partner company or device details
        company_name = (
            device.partner_company.company_name if device.partner_company else "Unknown"
        )
        ...
        usage_percentage = (
            (recent_usage / daily_average) * 100 if daily_average > 0 else 0
        )
        ...
        return {
            ...
            "alert_needed": (
                recent_usage < expected_70_percent_usage if daily_average > 0 else False
            ),
...
def check_all_devices_usage():
    ...
    try:
            ...
            devices_summary.append(
                {
                    ...
                    "usage_percentage": usage_percentage,
                    "alert_needed": (
                        recent_usage < expected_70_percent_usage
                        if daily_average > 0
                        else False
                    ),
                    "last_reading_time": latest_energy.timestamp.strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                }
            )
        ...
        return {
            ...
            "devices_needing_alert": len(
                [d for d in devices_summary if d["alert_needed"]]
            ),
            "period": f"Last 30 days ({start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')})",
