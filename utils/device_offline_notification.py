...
def check_existing_offline_devices():
    try:
        ...
        offline_devices = DeviceConfiguration.objects.filter(is_online=False, deleted=False).select_related(
            "partner_company"
        )
            ...
            if device.updated_at:
                ...
                if hours_offline >= 2:
                    ...
                    if last_notification:
                        ...
                        time_since_last_notification = (timezone.now() - last_notification).total_seconds() / 3600
                        if time_since_last_notification < 2:
                            ...
                            scheduled_count += 1
                            logger.info(f"Scheduled offline notification for device {device_identifier}")

        logger.info(f"Scheduled notifications for {scheduled_count} existing offline devices")
