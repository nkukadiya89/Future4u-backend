...
def send_device_config_request_email(subject, template, data):
    context = {
        "name": data.get("name"),
    ...
    custom_message_content = "A new device configuration request has been created.\n"
    ...
    try:
        ...
        try:
            ...
            try:
                mail_server = smtplib.SMTP_SSL("smtp.gmail.com", 465, timeout=60)
                mail_server.ehlo()
                mail_server.login(config("ADMIN_EMAIL"), config("EMAIL_PASSWORD"))
                ...
                try:
                    mail_server = smtplib.SMTP("smtp.gmail.com", 25, timeout=60)
                    mail_server.ehlo()
                    mail_server.starttls()
                    mail_server.ehlo()
                    mail_server.login(config("ADMIN_EMAIL"), config("EMAIL_PASSWORD"))
                    ...
                    error_msg = (
                        f"Email sending failed - STARTTLS: {str(tls_error)}, "
                        f"SSL: {str(ssl_error)}, Port25: {str(port25_error)}"
                    )
                    # Log failed email
                    log_email_failed(
