...
def send_confirm_mail(subject, template, data):
    ...
    msg = MIMEMultipart()
    msg["From"] = "PROCEM <" + config("ADMIN_EMAIL") + ">"
    ...
    url = os.path.join(BASE_DIR, "static/images/procemlogo.png")
...
def send_success_mail(subject, template, data):
    ...
    created_by_employee = user.employee.created_by.email if user.employee else None
    created_by_company = user.company.created_by.email if user.company else None
    created_by_vendor = user.vendor.created_by.email if user.vendor else None
