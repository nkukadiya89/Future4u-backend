...
def send_subscription_cart_reminder_email(cart_items, company, user_email):
    ...
    try:
        ...
        for item in cart_items:
            business_setting = BusinessSetting.objects.filter(
                company_id=company.id
            ).first()
            is_gujarat = bool(
                business_setting
                and business_setting.state
        ...
        logger.error(
            f"Failed to send subscription cart reminder email to {user_email}: {str(e)}"
        )
        log_email_failed(
...
def check_and_send_cart_reminders():
    ...
    try:
        ...
        try:
            carts_needing_reminder = (
                SubscriptionCart.objects.filter(
                    deleted=False,
                    created_at__lt=cutoff_time,
                )
                .filter(
                    Q(last_reminder_sent__lt=cutoff_time)
        ...
        for company_id, data in companies_with_carts.items():
            ...
            if user_email:
                ...
                success = send_subscription_cart_reminder_email(
                    cart_items=cart_items, company=company, user_email=user_email
                )
                ...
                if success:
                    ...
                    reminders_sent += 1
                    logger.info(
                        f"Sent cart reminder to company {company.name} ({user_email})"
                    )
                else:
                    logger.error(
                        f"Failed to send cart reminder to company {company.name}"
                    )
            else:
                logger.warning(f"No email address found for company {company.name}")

        logger.info(
            f"Cart reminder process completed. Sent {reminders_sent} reminders."
        )
